#if defined(LA_BITS) && defined(TP_DO_SX) && defined(IDENTSFX)

#define TAG_IN_PLACE 0

#define CAT0(x,y) x ## y
#define CAT(x,y) CAT0(x, y)
#define IDENT(x) CAT(x, IDENTSFX)
#define TAGH_BITS (64 - LA_BITS)
...
struct iel_tp_untag_st IDENT(utp) (void const *ptr, size_t al) {
    ...
    ptrval <<= TAGH_BITS;
    // XXX: assumes reinterpretation conversion + two's complement
...
static inline IEL_FNATTR_NODISCARD
uintmax_t IDENT(maxtag)(size_t al) {
    IEL_STATTR_ASSUME(!(al & (al - 1)));
    return (uintptr_t)al << TAGH_BITS;
}
